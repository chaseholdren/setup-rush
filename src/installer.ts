...
export async function getNode(
  ...
  if (checkLatest) {
    ...
    const resolvedVersion = await resolveVersionFromManifest(versionSpec, stable, auth, osArch, manifest);
    if (resolvedVersion) {
  ...
  if (toolPath) {
    ...
    if (!downloadPath) {
      ...
      try {
        ...
        if (err instanceof tc.HTTPError && err.httpStatusCode == 404) {
          return await acquireNodeFromFallbackLocation(info.resolvedVersion, info.arch);
        }
    ...
    toolPath = await tc.cacheDir(extPath, 'node', info.resolvedVersion, info.arch);
    core.info('Done');
  }
...
function resolveLtsAliasFromManifest(versionSpec: string, stable: boolean, manifest: INodeRelease[]): string {
...
async function getInfoFromDist(versionSpec: string, arch: string = os.arch()): Promise<INodeVersionInfo | null> {
  let osPlat: string = os.platform();
  let osArch: string = translateArchToDistUrl(arch);
  ...
  let fileName: string = osPlat == 'win32' ? `node-v${version}-win-${osArch}` : `node-v${version}-${osPlat}-${osArch}`;
  let urlFileName: string = osPlat == 'win32' ? `${fileName}.7z` : `${fileName}.tar.gz`;
...
async function queryDistForMatch(versionSpec: string, arch: string = os.arch()): Promise<string> {
  let osPlat: string = os.platform();
  let osArch: string = translateArchToDistUrl(arch);
...
async function acquireNodeFromFallbackLocation(version: string, arch: string = os.arch()): Promise<string> {
  let osPlat: string = os.platform();
  let osArch: string = translateArchToDistUrl(arch);
