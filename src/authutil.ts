...
function writeRegistryToFile(registryUrl: string, fileLocation: string, alwaysAuth: string) {
  ...
  const authString: string = registryUrl.replace(/(^\w+:|^)/, '') + ':_authToken=${NODE_AUTH_TOKEN}';
  const registryString: string = scope ? `${scope}:registry=${registryUrl}` : `registry=${registryUrl}`;
